        ...
        while (is_end(buffer) == 0){
            if(strcmp(buffer->name,path) == 0){
                if(is_dir(tar_fd,path)!=0){
                    size_t n_entries = *no_entries;
                    *no_entries=0;
                    tar_header_t* new_buf = (tar_header_t*)((uint8_t*)buffer + 512);
                    while(strncmp(buffer->name, new_buf->name, strlen(buffer->name)) == 0 && *no_entries < n_entries){
                        entries[*no_entries] = new_buf->name;
                        printf("%s\n",entries[*no_entries]);
                        (*no_entries)++;
                        if(new_buf->typeflag == AREGTYPE || new_buf->typeflag == REGTYPE){
                            new_buf = (tar_header_t*)((uint8_t*)new_buf + find_block(TAR_INT(new_buf->size))*512 + 512);
                        }
                        if(new_buf->typeflag == DIRTYPE){
                            char* name_dir=new_buf->name;
                            while(strncmp(name_dir,new_buf->name,strlen(name_dir))==0){
                                new_buf = (tar_header_t*)((uint8_t*)new_buf + 512);
                                if(new_buf->typeflag == AREGTYPE || new_buf->typeflag == REGTYPE){
                                    new_buf = (tar_header_t*)((uint8_t*)new_buf + find_block(TAR_INT(new_buf->size))*512 + 512);
                                }
                            }
